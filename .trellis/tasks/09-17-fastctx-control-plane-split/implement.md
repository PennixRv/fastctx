...
7. 更新版本、README/release notes 和 npm package identity；提交、推送 `PennixRv/fastctx`，创建修正版 `v0.2.10` GitHub release，并由 Actions 真实发布 scoped npm 包，记录资产、registry 版本与 commit。
...
cargo test
cargo build --release
gh release view v0.2.10 --repo PennixRv/fastctx
npm view @pennixrv/fastctx@0.2.10 version --registry https://registry.npmjs.org/
```
